            ...
            try {
                ...
                try {
                    ...
                    for (Entry<Allele, Collection<Genotype>> alleleToGenotypes : alleles.asMap().entrySet()) {
                        ...
                        vcb.attribute(infoPrefix + SAMPLES_CONTAINING_FREQUENCY_INFO, (double) samplesList.size()
                                / allSamplesCount);

                        vcb.attribute(infoPrefix + ALLELE_FREQUENCY_INFO, (double) genotypes.size()
                                / (referenceCount + genotypes.size()));
